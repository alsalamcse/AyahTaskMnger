...
////     }
    private void creatMyTask(MyTask t){
        ...
        reference.child("task").child(key).setValue(t).addOnCompleteListener(TempAllTaskActivity.this, new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful())
                {

                    Toast.makeText(TempAllTaskActivity.this, "add successful", Toast.LENGTH_SHORT).show();
                    finish();
                }
                else
                {
                    Toast.makeText(TempAllTaskActivity.this, "add failed", Toast.LENGTH_SHORT).show();
